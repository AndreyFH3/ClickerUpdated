...
public class saveSystem : MonoBehaviour
{
    ...
    private void onlineEarn()
    {
        ...
        if (YandexGame.savesData.language == "ru")
            offlineText.text = $"Денег заработано, пока вы были офлайн {ConverterMoney.Convert(earnedMoney)}";
        else 
            offlineText.text = $"Money earned while you were offline {ConverterMoney.Convert(earnedMoney)}";
    }
